...
public class QLearningAgent extends AbstractPlayer {
    ...
    public static double egreedyEpsilon = 0.05; // epsilon for epsilon-greedy action-selection
    public static double gamma = 0.8; // discount factor for future rewards
    public static double alpha = 0.4; // learning rate
    private static final double HUGE_POSITIVE =  10.0; // reward for reaching the goal
    private static final double DEFAULT_REWARD = -0.1; // reward for taking one step in the maze
	...
	public QLearningAgent(StateObservation gameState, ElapsedCpuTimer elapsedTimer) {
		randomGenerator = new Random();
		
		if (qValues == null) {
			// we are running the first episode and thus need to initialize everything.
			// note that everything that is declared static is common to all instances of the agent.
			
			// read the actions that are available in the game into a static array.
	...
	private double getReward(StateObservation gameState, int actIdx) {
		// use forward model to obtain action-outcome. this is a little hack for now, as we
		// have not found a hook to obtain the action-outcome without utilizing the forward model
        StateObservation nextState = gameState.copy();
        nextState.advance(actions[actIdx]);
        
        // if we encounter invalid actions on the way, we set their respective q-values to negative infinity
        if (nextState.getAvatarPosition().x == gameState.getAvatarPosition().x && 
        	nextState.getAvatarPosition().y == gameState.getAvatarPosition().y) {
        	return Double.NEGATIVE_INFINITY;
        }
        
        // if action was valid, but we did not reach the goal, yield default reward.
	...
	private int getBestActionIdx(int stateX, int stateY) {
		...
		for (int i=1; i < nActions; i++) {
			if (qValues[stateX][stateY][i] > qValues[stateX][stateY][maxIdx]) {
				maxIdx = i;
			} else if (qValues[stateX][stateY][i] == qValues[stateX][stateY][maxIdx] && randomGenerator.nextInt(2) == 1) {
				// randomly pick the new idx half the time when its qvalue is equal to the known max so far
				maxIdx = i;
			}
		}
		return maxIdx;
	...
	private double getQPrediction(int stateX, int stateY, int actIdx) {
        ...
        double newVal = gamma * getBestQValue(nextX, nextY);
        return newVal;
	}
	...
	public ACTIONS act(StateObservation gameState, ElapsedCpuTimer elapsedTimer) {
        ...
        qValues[stateX][stateY][actIdx] = (1 - alpha) * oldVal + alpha * (reward + nextQVal);
        
        //System.out.println("ACTION: (" + stateY + "," + stateX + ") to (" + nextY + "," + nextX + "), update: " + oldVal + "->" + newVal);
