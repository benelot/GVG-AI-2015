...
public class ITypeAttractivity extends HashMap<Integer,Double> {
	 ...
	 * 			   Maps Category to a prior Attraction Value, contains Avatar=0, Resource=1, Portal=2, NPC=3, Static=4, FromAvatar=5, Movable=6
	 *             
	 */
	 ...
	 *             a StateObservation
	 * @param SpriteCategoryPriors
	 * 			   Array of Attraction Values, in the following order Avatar=0, Resource=1, Portal=2, NPC=3, Static=4, FromAvatar=5, Movable=6
	 *             
	 */
	public ITypeAttractivity(StateObservation StateObs, Double[] SpriteCategoryPriors){
		...
		//Set prior Attraction values of categories
		SpriteCategoryValue = new HashMap<Integer,Double>();
		SpriteCategoryValue.put(Types.TYPE_AVATAR, SpriteCategoryPriors[Types.TYPE_AVATAR]);
		SpriteCategoryValue.put(Types.TYPE_RESOURCE, SpriteCategoryPriors[Types.TYPE_RESOURCE]);
		SpriteCategoryValue.put(Types.TYPE_PORTAL, SpriteCategoryPriors[Types.TYPE_PORTAL]);
		SpriteCategoryValue.put(Types.TYPE_NPC, SpriteCategoryPriors[Types.TYPE_NPC]);
		SpriteCategoryValue.put(Types.TYPE_STATIC,SpriteCategoryPriors[Types.TYPE_STATIC]);
		SpriteCategoryValue.put(Types.TYPE_FROMAVATAR, SpriteCategoryPriors[Types.TYPE_FROMAVATAR]);
		SpriteCategoryValue.put(Types.TYPE_MOVABLE, SpriteCategoryPriors[Types.TYPE_MOVABLE]);
	...
	/**
	 * updates the itypesAttractionValues based on a state Observation
	 * TODO: Maybe it would be good to make a difference between Avatar and FromAvatar Events.
	 * TODO: Maybe there is a better learning rule
	 * TODO: The updateVals have to be adjusted
	...
	public void updateAttraction (StateObservation stateObs, Double previousScore) {	
		...
		while (eventIterator.hasNext()) {
			...
			// it seems not to be possible to get the corresponding Observation (and the category), so any unknown iType is initialized with 0
			...
			if (newVal>1){newVal=1;}
			//System.out.println("change attraction value of iType" + currEvent.passiveTypeId + "from" + A this.get(currEvent.passiveTypeId) + "to" + newVal );
