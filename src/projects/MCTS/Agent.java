...
public class Agent extends AbstractPlayer {
	...
	//running and fixed MCTS_DEPTH, first increments to counter the increment of the depth of the cut trees. The later stays fixed
	public static int MCTS_DEPTH_RUN ;
	public static int MCTS_DEPTH_FIX = 3;
	public static double K = Math.sqrt(2);
	public static Types.ACTIONS[] actions;

	// an exploration reward map that is laid over the game-world to reward places that haven't been visited lately
	...
	public Types.ACTIONS act(StateObservation stateObs, ElapsedCpuTimer elapsedTimer) {
		...
		// Heuristic: change the reward in the exploration reward map of the visited current position
		Vector2d pos = stateObs.getAvatarPosition();
		Dimension dim = stateObs.getWorldDimension();
		int intposX = (int) Math.round(pos.x/dim.getWidth() * (rewMapResolution-1));
		int intposY = (int) Math.round(pos.y/dim.getHeight() * (rewMapResolution-1));
		//addRewMap[intposX][intposY] /= 2;
		//if(addRewMap[intposX][intposY] < 0.01){
		//addRewMap[intposX][intposY]  = -0.1;}

		// increment reward at all unvisited positions and decrement at current position 
		for( int i = 0; i < rewMapResolution; i++){
			for( int j = 0; j < rewMapResolution; j++){
				if(addRewMap[i][j]< 1){
					addRewMap[i][j] += 0.001;
		...
		// System.out.println("Current Position, x: "+intposX +"  y : "+ intposY + "  with val " + addRewMap[intposX][intposY] );
		...
		// Heuristic (IDEA): Perhaps increase reward towards positions that have a resource. -> some sort of diffusion model with the resources as positive 
		...
		if( useOldTree == 1){
			...
			//Set the state observation object as the new root of the tree. (Forgets the whole tree)
